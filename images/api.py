...
from .media_constants import ACCEPTABLE_VALUES
from .serializers import JWTTokenSerializer, ImageSerializer
...
class FileUploadView(views.APIView):
    ...
    def post(self, request, domain):
        ...
        return Response(data={'image': upload_path}, status=status.HTTP_201_CREATED)
...
class JWTTokenView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if file_size > ACCEPTABLE_VALUES[domain]['size']:
            raise ValidationError({"error": "Exceeded maximum size."})
